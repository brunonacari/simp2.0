 ...
 * Endpoint: Buscar Registros de um Dia e Ponto de Medição Específico
 */
...
try {
    ...
    $where = "WHERE CONVERT(DATE, RVP.DT_LEITURA) = :data";
    $params = [':data' => $data];

    // Filtro por Ponto de Medição (obrigatório quando não filtrou por ponto na busca inicial)
    if (!empty($cdPontoMedicao)) {
        $where .= " AND RVP.CD_PONTO_MEDICAO = :cd_ponto_medicao";
    ...
    // Verificar se é para retornar apenas as chaves (para exclusão em massa)
    ...
    if ($apenasChaves) {
        ...
        // Buscar registros completos do dia
        $sql = "SELECT 
                RVP.CD_CHAVE,
    ...
    echo json_encode([
        ...
        'ponto_consultado' => $cdPontoMedicao
    ]);
    }
} catch (Exception $e) {
